...
The above program uses a simple update process that relies on storing the current state of the robot and adjusting this in each update step based on a direction vector. As noted above the only collision detection is with the invisible walls of the room (based on a simple 2D test). The robots do not collide with each other. Also, robot collision with the wall is a simple test using the centre coordinate of the robot rather than any of its extremities. In practice, a bounding box might be used to test against extremities, or the actual pieces of the robot might be tested. The latter would be time consuming if the robot was intricately modelled.

Another consideration is the update process itself. In the program above, an update step is determined by stored values. An alternative would be a scripted animation curve (or a set or curves, with one for each changing parameter). Here, the robot would follow a scripted path from A to B in a set period of time. If all paths are scripted, then the animator is responsible for making sure nothing collides with anything else and, essentially, a storyboarded animation sequence is being produced, of the kind that might be done for an animated film (and for which the simulation and rendering steps can take seconds, minutes, hours or longer). This contrasts with the approach above, where the result is unknown, since collisions can change an object's motion path.

Another alternative is a mixture of the two, i.e. some movements based on stored parameters and update steps like the robot in SG07 and potentially affected by user input and some based on pre-scripted paths. Considering this demonstrates the complexity of any animation work.
