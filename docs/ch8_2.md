...
1. The initialise method in Program Listing 7.4 uses a lot of literals (e.g. scale(2,4,2)). This makes it difficult to change one without considering the knock-on effect for other transformations in the tree. Change the method so that variables are used instead of literals in those places where this would increase understanding, flexibility and maintainability. [Solution](#exercise-solutions)

2. Add another cube on top of the upper cube (this is similar to exercise 1 in section 7.1). Using variables instead of literals in the initialise method will make this easier. Figure 7.6 shows a possible solution. [Solution](#exercise-solutions)
...
<details>
<summary>a) It stores the transformation matrix for a specific model.</summary>
<p><b>Incorrect.</b> The SGTransformNode class is responsible for storing and applying transformation matrices to the scene graph.</p>
</details>

<details>
<summary>b) It solely provides a name to a node to make the hierarchy clearer.</summary>
<p><b>Incorrect.</b> This is the purpose of the SGNameNode class.</p>
</details>

<details>
<summary>c) It contains a Model instance and is responsible for calling its render method with the correct world transformation.</summary>
<p><b>Correct.</b> As stated in the text and shown in Program Listing 8.6, the SGModelNode contains a Model instance and its draw method calls model.render(gl, worldTransform).</p>
</details>

<details>
<summary>d) It is the root of the entire scene graph and holds all other nodes.</summary>
<p><b>Incorrect.</b> The root is a type of SGNode (in this case, an SGNameNode called stackRoot), but SGModelNode is specifically used to represent an object to be rendered.</p>
</details>
...
<details>
<summary>a) It is a required standard for all models, regardless of whether a scene graph is used.</summary>
<p><b>Incorrect.</b> The previous example (SG01) shows that transformations were applied directly to the models when a scene graph was not used.</p>
</details>

<details>
<summary>b) Because the scene graph classes will handle all transformations, making the transformation stored inside the Model class superfluous.</p>
<p><b>Correct.</b> The text explicitly states, "This is because we are going to use the scene graph to control transformations, so the transformation stored inside the Model class is superfluous." The scene graph's draw method passes the correct world transformation to the Model instance's render method, overriding any local transformation it might have.</p>
</details>

<details>
<summary>c) It is a way to ensure that the models are rendered at the origin (0,0,0) before any transformations are applied.</p>
<p><b>Incorrect.</b> While an identity matrix places the model at the origin, the scene graph then applies a series of transformations to place the model in its correct world position. The identity matrix is simply a starting point because the scene graph takes over the transformation responsibility.</p>
</details>

<details>
<summary>d) It is done for debugging purposes to make sure the program is running correctly before applying any transformations.</p>
<p><b>Incorrect.</b> The use of an identity matrix is part of the final, working implementation, not just for debugging. The SGNode.print() method is mentioned as a debugging tool.</p>
</details>
...
<details>
<summary>a) It is calculated by adding the transformations of the parent and child nodes.</summary>
<p><b>Incorrect.</b> Transformations are matrices and are combined through multiplication, not addition.</p>
</details>

<details>
<summary>b) It is calculated by multiplying the parent's world transformation with the node's local transformation.</p>
<p><b>Correct.</b> As shown in the update() method of SGTransformNode (t = Mat4.multiply(worldTransform, transform)), the new world transformation (t) is a result of multiplying the parent's world transformation (worldTransform) with the current node's local transformation (transform).</p>
</details>

<details>
<summary>c) The final transformation is stored directly within the SGNode and is not calculated dynamically.</p>
<p><b>Incorrect.</b> The worldTransform is stored in the SGNode but it is calculated dynamically by the update() method, which traverses the scene graph from the root down to the leaves.</p>
</details>

<details>
<summary>d) The SGTransformNode takes a copy of the parent's transform and applies it directly to the children.</p>
<p><b>Incorrect.</b> The SGTransformNode first multiplies the parent's transform by its own local transform and then passes that new, combined transform to its children.</p>
</details>
...
2. Add another cube on top of the upper cube (this is similar to exercise 1 in section 7.1). Using variables instead of literals in the initialise method will make this easier. Figure 7.6 shows a possible solution.
