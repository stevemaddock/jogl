...
A separate Cube class and Sphere class are created which can be used to supply the vertex and index lists to the Mesh class, as illustrated in Program Listings 6.10 and 6.11. The arrays are stored as public static final attributes so can be easily accessed, e.g. Cube.vertices.
