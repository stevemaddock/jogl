...
The result from this section is a scene that contains a sphere and a light, as illustrated in Figure 6.6.

<p align="center">
  <img src="ch6_img/ch6_mesh.png" alt="a plain sphere" width="300"><br>
...
A separate Cube class and Sphere class are created which can be used to supply the vertex and index lists to the Mesh class, as illustrated in Program Listings 6.10 and 6.11. The arrays are stored as public static final attributes so can be easily accessed, e.g. Cube.vertices;. The Cube and SPhere classes could be enhanced to implement the Cloneable interface and then the line could be rewritten as Cube.vertices.clone().
...
Program Listing 6.12 gives an example that declares two instances of the Mesh class. Here, both a cube and a light object are declared. This gives the possibility of representing the light as any object. Here, a sphere is used for the light. 

The program that is supplied alters the line ```cube = new Mesh(gl, Cube.vertices, Cube.indices);``` to ```cube = new Mesh(gl, Sphere.vertices, Sphere.indices);``` in order to produce the output in Figure 6.6. THis should instead have been stored in a avraible called sphere to avoid confusion.
...
1. Try adding a second object, e.g. a cube, to the scene.

2. Experiment with changing the material properties of the cube or sphere and the light. What impact does this have on the final rendering? (It can be difficult to predict the results when a non-white light is used.)
...
```java
  private void updateLightColour() {
    ...
    //... use lightColour to change ambient and diffuse values for the light
