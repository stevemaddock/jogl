...
public void initialise(GL3 gl) {
  ...
  SGTransformNode translateToTop = new SGTransformNode("translate(0,1,0)",Mat4Transform.translate(0,0,0));
...
1. One of the attributes in the class in Program Listing 8.14 is xPosition, which is used in translateX. Using the solution for the previous exercise, add two extra buttons to the main program (SG05.java) to increment and decrement the xPosition value by a certain amount. This will entail adding extra methods to SG05_GLEventListener.java to carry out these actions and redraw the scene.
...
A new menu button is added to SG04.java:

```java
b = new JButton("new F position");
...
```java
else if (e.getActionCommand().equalsIgnoreCase("new F position")) {
  glEventListener.newFPosition();
}
```

which calls the method newFPosition that is added to SG04_GLEventListener.java

```java
public void newFPosition() {
  float x = (float)(10 * Math.random() - 5f);
  float z = (float)(10 * Math.random() - 5f);
  fObj.setPosition(new Vec3(x,0f,z));
}
```

which in turn calls a new method in class FObject:

```java
public void setPosition(Vec3 position) {
  System.out.println("new position: "+position);
  translateFInPlane.setTransform(Mat4Transform.translate(position.x, position.y, position.z));
  fShapeRoot.update();
}
```

This also requires that translateFInPlane is declared at the class level, not in the makeSG method:

```java
private SGTransformNode translateFInPlane;
```
