...
S05_GLEventListener.java (see Program Listing 3.13 for some of this) makes use of the Shader class. A private variable called shader of type Shader is declared as an attribute of the class and an instance of this is created in method initialise(), which also loads the text files vs_S05.txt and fs_S05.txt (Program Listings 3.14 and 3.15, respectively) which are the vertex and fragment shader source code, respectively. Method render() can then use the shader variable with the call shader.use(gl). The output of S05 is shown in Figure 3.5. 

I have chosen to use the .txt extension with all my vertex and fragment shaders. This is not standard. Typically, you will see them called name.vert and name.frag. The advantage of using .vert and .frag is that you can easily get syntax highlighting in editors such as vscode. I'll stick with .txt extensions for this tutorial, since this at least labels then as plain text files.
...
1. Adjust the vertex shader (vs_S05.txt in Program Listing 3.14) so that the triangle is drawn upside down. (Hint: This can be done by manipulating the position values in the vertex shader when assigning them to gl_Position rather than by editing the data section of the main program.) [Solution](#exercise-solutions)

2. Specify a horizontal offset via a uniform and move the triangle to the right side of the screen in the vertex shader using this offset value. (Hint: add some constant offset values in the vertex shader or declare a uniform and fill in the values by linking to this in the render method.) [Solution](#exercise-solutions)
...
1. Adjust the vertex shader (vs_S05.txt in Program Listing 3.14) so that the triangle is drawn upside down. (Hint: This can be done by manipulating the position values in the vertex shader when assigning them to gl_Position rather than by editing the data section of the main program.)

Given that the triangle has vertices above and below the x axis,
negating the y coordinate is a simple solution to draw the triangle upside down.
Of course, this relies on us knowing the form of the original data.
However, the solution suffices to demonstrate the aims of this exercise, which are:
(i) the coordinates that are supplied to a vertex shader can be manipulated;
(ii) the vertex shader can be changed in any text editor, 
and the program does not have to be recompiled to use it.
...
2. Specify a horizontal offset via a uniform and move the triangle to the right side of the screen in the vertex shader using this offset value. (Hint: add some constant offset values in the vertex shader or declare a uniform and fill in the values by linking to this in the render method.)

This is similar to the exercise in Section 3.1. However, now the Shader class
is used, which means the render method needs updating as well as the vertex shader.
...
public void render(GL3 gl) {
  ...
  float xOffset = 0.4f;
  float yOffset = 0.0f;
  shader.setFloat(gl, "uniformOffset", xOffset, yOffset);  // call to method in Shader class
  // could instead only pass a single float rather than two floats
  // uniform variable in the shader would then be a float, not a vec2
...
void main() {
  gl_Position = vec4(position.x+uniformOffset.x, position.y, position.z, 1.0);
  aColor = color;
}
```
