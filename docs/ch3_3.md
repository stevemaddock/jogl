...
In all the previous programs, the code to set up the shaders has remained unchanged, even if the source for the vertex and fragment shaders has changed. It makes sense to separate the shader code out into a separate class and load the shaders from file. The new class is Shader.java (see Program listing 3.12 for part of this). The new code is then used in S05_GLEventListener.java (Program Listing 3.13 for some of this) and the text files vs_S05.txt and fs_S05.txt (Program Listings 3.14 and 3.15, respectively) are used for the vertex and fragment shader source code, respectively. The output of S05 is shown in Figure 3.7.

The majority of the code in Program Listing 3.12 is the same as previous examples, namely method compileAndLink(). The constructor loads the source code from the relevant text files. S05_GLEventListener.java makes use of the Shader class. A private variable called shader of type Shader is declared as an attribute of the class and an instance of this is created in method initialise(). Method render() can then use this with the call shader.use(gl). There are also some extra methods in Shader.java which can be used to set particular uniform values in the shaders. 
...
1. Adjust the vertex shader (vs_S05.txt in Program Listing 3.14) so that the triangle is drawn upside down. (Hint: This can be done by manipulating the position values in the vertex shader when assigning them to gl_Position.) [Solution]
