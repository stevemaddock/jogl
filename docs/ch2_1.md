...
In brief, the z buffer is used for hidden surface removal, where one surface should appear in front of another surface, or part of a surface in the case of intersecting surfaces. We will deal with the z buffer in detail in a later chapter.</p>
...
Program Listing 2.4:</span> A01_GLEventListener.java
...
- Alter the background colour of the screen. (Hint: consider `glClearColor()` in `init()`.) [<a href="solutions/ch2_1_e2.txt" target="_blank">Solution</a>]