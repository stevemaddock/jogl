...
2. Alter the background colour of the screen. (Hint: consider `glClearColor()` in `init()`.) [Solution](solutions/ch2_1_e2.txt)
...
<p><strong>:smile: Correct:</strong> A <code>GLDrawable</code> is a crucial abstraction in JOGL. It ties together the platform-specific windowing system's drawing area with the OpenGL state and functionalities needed to render graphics onto that area. Think of it as the canvas on which you paint with OpenGL.</p>
</details>
