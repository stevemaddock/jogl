...
The following line can be added between lines 17 and 18:

`animator.setUpdateFPSFrames(200, System.out);`

This will print out the frames per second to the command window (or terminal) every 2 seconds. Surprisingly, this will equate to 30 fps rather than 60fps on some machines. Do not worry about this at this stage. There is an alternative way to handle the display update that uses an `Animator` instead of an `FPSAnimator` that can be used to run a program as fast as possible (see the following [Discussion](https://forum.jogamp.org/FPSAnimator-Vs-Animator-td4025734.html)). We'll stick with FPSAnimator as it will suffice for our needs.
