...
1. Add an extra transformation into Program V04 so that each cube is a random size. [Solution](#exercise-solutions)
...
1. Add an extra transformation into Program V04 so that each cube is a random size. 

Focus on the method: private Mat4 getModelMatrix(int i)

An extra modelMatrix multiplication needs to be included to scale the cube, e.g.

float xScale = ...random value in a specific range
float yScale = ...random value in a specific range
float zScale = ...random value in a specific range
modelMatrix = Mat4.multiply(modelMatrix, Mat4Transform.translate(x,y,z));
modelMatrix = Mat4.multiply(modelMatrix, Mat4Transform.rotateAroundY(yAngle));
modelMatrix = Mat4.multiply(modelMatrix, Mat4Transform.scale(xScale, yScale, zScale));
