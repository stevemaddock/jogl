...
1. You can alter the position and target point for the camera to look at, which can be useful in initially setting up a scene and viewing it from the 'best' position. Experiment with these by setting the values inside the constructor for V05_GLEventListener, e.g. this.camera.setPosition(new Vec3(4f,6f,15f)); this.camera.setTarget(new Vec3(0f,5f,0f));
