...
**Program Listing 6.18:** Initialising the scene
...
1. Replace the diffuse, specular and emission maps with some textures of your own.
...
`getSphereModelMatrix1` is shown in Program Listing 6.22. This is because there is an important rotation matrix included in its display. This is because of the way that texture coordinates are set up on the sphere. The u=0 and v=0 values start at +x and z=0 and then wrap anticlockwise around the y axis to join back up at +x and z=0 (see Sphere.java). Thus, if the texture was displayed on the untransformed sphere, it would be pointing in the -x direction. Rotating the sphere by 180 degrees solves this.

---

## Exercises

1. Change the if test in L05 so that the expression switches multiple times based on both the x and z coordinate values instead of just the z coordinate value. [Solution](#exercise-solutions)
...
<details>
<summary>d) They are a more efficient way to calculate lighting than using standard color values.</summary>
...
<details>
<summary>c) To quantize the color space, reducing the number of distinct colors used to render the final image.</summary>
...
<details>
<summary>c) It is controlled by the levels uniform, which is set by a slider in the user interface.</summary>
...
2. Change the if test in L05 so that the expression switches multiple times based on both the x and z coordinate values instead of just the z coordinate value.

There are multiple ways to do this. Here's one:

if (lightPosition.x > 0 && lightPosition.z > 0) {
    sphereDiffuseMap = sphereSmileDiffuseMap;
}
else (lightPosition.x <= 0 && lightPosition.z > 0) {
    sphereDiffuseMap = sphereFrownDiffuseMap;
}
else (lightPosition.x <= 0 && lightPosition.z <= 0) {
    sphereDiffuseMap = sphereSmileDiffuseMap;
