...
1. Create two new textures, one with a smile and one with a frown, to replace the ones in L05.

2. Change the if test in L05 so that the expression switches multiple times based on both the x and z coordinate values instead of just the z coordinate value. [Solution](#exercise-solutions)
