...
**Program listing 2.16:** Setting up the buffers with the addition of EBOs
...
Program Listing 2.16 has successfully set up the buffers on the GPU and described how the data is organised and a VAO is used to wrap all this. In the next section, the triangle can be repeatedly drawn by using the VAO using its ID: `vertexArrayId`.

---

### 2.3.3 Rendering

Program listing 2.17 shows the changes necessary to the `render` method to use EBOs. The `render` method from Section 2.2 is shown in Program Listing 2.18 for comparison purposes - only one line is different. `glDrawElements` is now used instead of `glDrawArrays`. This indicates that an index list is being used to decide how to access the vertex list.
...
**Program listing 2.17:** Using EBOs for rendering
...
**Program listing 2.18:** The render method from Section 2.2 - compare this with Program Listing 2.17. The only change is the method used to draw the data.
...
If you'd like further explanation of how EBO's work, then check out [Joey's Hello Triangle example](https://learnopengl.com/Getting-started/Hello-Triangle).
...
<p align="center">
  <img src="ch2_img/ch2_11_on_the_gpu.png" alt="Multiple copies of the shaders handle the data on the GPU" width="500"><br>
  <strong>Figure 2.11.</strong> Multiple copies of the shaders handle the data on the GPU
</p>

Figure 2.11 is an updated version of Figure 2.9 from section 2.2. It shows that we now have an EBO on the GPU that stores the index data. The vertex data is bound to this index data - see Program Listing 2.16. The GPU makes use of this information to decide how many copies of the vertex shader to make as it deals with the data in its buffers.

---

## Exercises

1. Instead of filling the triangle that is drawn, a 'wireframe' of it can be drawn by configuring OpenGL to draw lines for the edges of triangles instead of filling the triangles (Figure 2.12). This is done inside method `init`, where other OpenGL attributes are also similarly configured, e.g. setting the background clear colour. In JOGL, `gl.glPolygonMode(GL.GL_FRONT_AND_BACK, GL3.GL_LINE);` is used. Add this to the method `init` to see the effect on drawing the triangle.  

<p align="center">
  <img src="ch2_img/ebo_two_triangles_wireframe.jpg" alt="Two wireframe triangles" width="300"><br>
  <strong>Figure 2.12.</strong> Two wireframe triangles
</p>
     
Notes: (i) The default drawing mode is `glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);` (ii) Some OpenGL attributes can be referred to in JOGL using GL.XYZ. This applies to attributes that have been available in all previous versions of OpenGL. However, some require a specific OpenGL profile to be used, e.g. GL3.GL_LINE. In most cases, you can resort to just using GL3.XYZ for all of them, unless you are using a later OpenGL 4.x profile.

2. Change the program to draw three triangles. [Solution.] (Hint: it might be easier to see what is happening if you draw the outlines of the triangles, as in Figure 2.12.) (We'll look at how to change the colour of the triangles in the next chapter.) [Solution](#exercise-solutions)
...
All that needs to be changed is the data section.
